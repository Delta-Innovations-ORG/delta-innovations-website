# Contact & Online Presence

## Primary contact (website)

| Type | Value |
|------|-------|
| **Contact email** | deltainnovations.co@gmail.com |
| **Projects / insider** | insider.daltainnovations@gmail.com |

## Phone & WhatsApp

| Region | Call | WhatsApp |
|--------|------|----------|
| Pakistan | +923322795419 | +923322795419 |
| Egypt | +201002637979 | +201034025254 |

## Locations

Pakistan · Egypt

## Social & code

| Platform | URL |
|----------|-----|
| Website (live) | https://delta-innovations-website.vercel.app |
| Website (custom domain, when DNS live) | https://deltainnovations.net/ |
| GitHub org | https://github.com/Delta-Innovations-ORG |
| LinkedIn | https://www.linkedin.com/company/deltainnovations |
| This repository | https://github.com/Delta-Innovations-ORG/delta-innovations-website |

> **LinkedIn:** Use the Vercel URL on the company page Website field until the custom domain resolves. See [LinkedIn page guide](../05-github-and-growth/LINKEDIN_PAGE_GUIDE.md).

## Website contact routes

| Page | Path |
|------|------|
| Contact form | `/contact` |
| Project requirements | `/requirements` |
| Change requests | `/change-requests` |

## Updating contact details

Edit [`src/content/siteConfig.ts`](../../src/content/siteConfig.ts) — emails, phones, and social links are centralized there.

## Related docs

- [EmailJS setup](../02-website/EMAILJS_SETUP.md)  
- [GitHub repo setup](../05-github-and-growth/GITHUB_REPO_SETUP_GUIDE.md)
